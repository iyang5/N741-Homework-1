---
title: "N 741 - Spring 2018 - Homework #1"
author: "Irene Yang"
...
1. Created a Github repository for N741 Homework 1. Initialized it with a README file.
2. In RStudio, created a new project for N741 Homework 1.
3. Linked RStudio to my Github repository for N741 homework 1.
...
Mean life expectancy is `r round(mean(gapminder$lifeExp), digits=2)` years. 
...
Standard deviation for life expectancy is `r round(sd(gapminder$lifeExp), digits=2)`, median is `r round(median(gapminder$lifeExp), digits=2)`, and the sample size for this variable is `r round(length(gapminder$lifeExp), digits=2)`.
...
```{r}
aggregate(lifeExp ~ year, gapminder, mean)
```

```{r}
aggregate(lifeExp ~ year, gapminder, sd)
```
